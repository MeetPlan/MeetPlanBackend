...
func (db *sqlImpl) InsertAbsence(absence Absence) error {
	_, err := db.db.NamedExec(
		"INSERT INTO absence (id, user_id, teacher_id, meeting_id, absence_type) VALUES (:id, :user_id, :teacher_id, :meeting_id, :absence_type)",
		absence)
	return err
}
...
func (db *sqlImpl) UpdateAbsence(absence Absence) error {
	_, err := db.db.NamedExec(
		"UPDATE absence SET user_id=:user_id, teacher_id=:teacher_id, meeting_id=:meeting_id, absence_type=:absence_type WHERE id=:id",
		absence)
	return err
}
